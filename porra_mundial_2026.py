        ...
        if not st.session_state.admin:
            ...
            if ligas_disp:
                ...
                if st.button("🚀 ENTRAR A LA PORRA", use_container_width=True):
                    ...
                    st.query_params["liga"] = liga_seleccionada 
                    st.rerun()
            ...
            if st.button("➕ CREAR Y ENTRAR", use_container_width=True):
                if nueva_liga_input:
                    ...
                    st.query_params["liga"] = nueva_liga_input 
                    st.rerun()
...
def obtener_clasificados(df):
    ...
    for i in range(8):
        ...
        if k in resultados_grupos:
            eq_manual = resultados_grupos[k].get('equipo_A')
            if eq_manual: pos[f"3_{i+1}"] = eq_manual
...
def calcular_puntos(df):
    ...
    for g in GRUPOS.keys():
        part = [p for p in resultados_grupos.values() if p['equipo_A'] in GRUPOS[g] and p.get('equipo_B') != ""]
        eqs = df[df['Grupo']==g].to_dict('records')
...
if menu == "🔄 Cambiar de Liga":
    ...
    if ligas_todas:
        ...
        if st.button("🚀 Saltar a esta Liga", use_container_width=True):
            ...
            st.query_params["liga"] = liga_salto
            st.session_state.menu_seleccionado = "📊 Clasificación General"
            st.rerun()
    else:
                ...
                for g, equipos in GRUPOS.items():
                    for eA, eB in [(equipos[0],equipos[1]), (equipos[2],equipos[3]), (equipos[0],equipos[2]), (equipos[1],equipos[3]), (equipos[0],equipos[3]), (equipos[1],equipos[2])]:
                        if eA in row["Equipos"] or eB in row["Equipos"]:
                            ...
                            if k in resultados_grupos and resultados_grupos[k].get('equipo_B') != "":
    ...
    for g, equipos in GRUPOS.items():
        with [c1, c2, c3][col_idx % 3]:
            ...
            for eA, eB in [(equipos[0],equipos[1]), (equipos[2],equipos[3]), (equipos[0],equipos[2]), (equipos[1],equipos[3]), (equipos[0],equipos[3]), (equipos[1],equipos[2])]:
                ...
                if k in resultados_grupos and resultados_grupos[k].get('equipo_B') != "": html_p += f'<div style="border-bottom:1px solid #2d3748; padding:6px 0;"><div style="font-size:0.65em; color:#aaa; text-align:center;">🕒 {f}</div><div style="display:flex; justify-content:space-between; align-items:center;"><span style="color:white; width:40%; text-align:right; font-size:0.9em;">{flag(eA)} {eA}</span><span style="background:#FFD700; color:#000; padding:2px 8px; border-radius:4px; font-weight:bold;">{resultados_grupos[k]["goles_A"]}-{resultados_grupos[k]["goles_B"]}</span><span style="color:white; width:40%; text-align:left; font-size:0.9em;">{eB} {flag(eB)}</span></div></div>'
...
# ══════════════════════════════════════════
    ...
    for g in GRUPOS.keys():
        ...
        if len(eqs)==4: ter_base.append(eqs[2])
    ter_base=sorted(ter_base,key=lambda x:(x['Pts'],x['Dif'],x['GF']),reverse=True)
    ter_nombres = [t['Equipo'] for t in ter_base[:8]]
    
    if not ter_nombres:
        st.warning("Aún no se pueden calcular los terceros. Rellena los resultados de los grupos primero.")
    else:
        slots_info = {
            "3_1": "M74 (vs 1º Grupo E)", "3_2": "M77 (vs 1º Grupo I)", "3_3": "M79 (vs 1º Grupo A)", 
            "3_4": "M80 (vs 1º Grupo L)", "3_5": "M81 (vs 1º Grupo D)", "3_6": "M82 (vs 1º Grupo G)", 
            "3_7": "M85 (vs 1º Grupo B)", "3_8": "M87 (vs 1º Grupo K)"
        }
        
        with st.form("form_terceros"):
            selecciones = {}
            for slot, desc in slots_info.items():
                k = f"ruta_{slot}"
                ...
                if eq_guardado in ter_nombres:
                    ...
                    idx = list(slots_info.keys()).index(slot)
                    if idx >= len(ter_nombres): idx = 0
                    
                selecciones[slot] = st.selectbox(f"Hueco {slot} -> Jugará el partido {desc}", ter_nombres, index=idx)
                
            if st.form_submit_button("💾 Guardar Cruces Definitivos"):
                if len(set(selecciones.values())) < len(ter_nombres):
    ...
    if check:
        if st.button("💥 BORRAR TODOS LOS RESULTADOS DEL TORNEO 💥", use_container_width=True):
            ...
            cargar_resultados_elim.clear()
            cargar_pichichis_reales.clear()
            st.rerun()    .subtitulo { text-align: center; color: #aaa !important; font-size: clamp(0.8em, 3vw, 1.2em); margin-bottom: 20px; letter-spacing: 5px; }
