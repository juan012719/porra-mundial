...
def cargar_resultados_grupos():
    ...
    for r in rows:
        result[r["key"]] = {
            "equipo_A": r["equipo_a"], "equipo_B": r["equipo_b"],
            "goles_A": r["goles_a"], "goles_B": r["goles_b"]
        }
    return result
...
def cargar_resultados_elim():
    ...
    for r in rows:
        result[r["key"]] = {
            "equipo_A": r["equipo_a"], "equipo_B": r["equipo_b"],
            "goles_A": r["goles_a"], "goles_B": r["goles_b"],
            "resolucion": r["resolucion"], "ganador": r["ganador"], "perdedor": r["perdedor"]
        }
    return result
...
def guardar_participantes(participantes):
    ...
    if rows:
        sb.table("participantes").insert(rows).execute()
    cargar_participantes.clear()

def guardar_resultado_grupo(key, eA, eB, gA, gB):
    sb = get_supabase()
    sb.table("resultados_grupos").upsert({
        "key": key, "equipo_a": eA, "equipo_b": eB, "goles_a": gA, "goles_b": gB
    }).execute()
...
def guardar_resultado_elim(m_id, eA, eB, gA, gB, res, gan, perd):
    sb = get_supabase()
    sb.table("resultados_elim").upsert({
        "key": m_id, "equipo_a": eA, "equipo_b": eB,
        "goles_a": gA, "goles_b": gB,
        "resolucion": res, "ganador": gan, "perdedor": perd
    }).execute()
...
def guardar_pichichi(eq):
    ...
    if eq:
        sb.table("pichichi").insert({"equipo": eq}).execute()
